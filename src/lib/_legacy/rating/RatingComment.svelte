<script lang="ts">
  ...
  let neutralStars: number = comment.total - roundedRating;
</script>
...
<article>
  <div class="flex items-center mb-4 space-x-4">
    ...
    <div class="space-y-1 font-medium dark:text-white">
      <p>
        {comment.user.name}
        <time datetime="2014-08-16 19:00" class="block text-sm text-neutral-500 dark:text-neutral-400">
          {comment.user.joined}
        </time>
      </p>
    </div>
  ...
  <div class="flex items-center mb-1">
    <Rating total={comment.total} rating={comment.rating}>
      <p slot="text" class="ml-2 text-sm font-medium text-neutral-500 dark:text-neutral-400">
        {comment.rating} out of {comment.total}
      </p>
    </Rating>
    {#if comment.heading}
      <h3 class="ml-2 text-sm font-semibold text-neutral-900 dark:text-white">
        {comment.heading}
      </h3>
    {/if}
  </div>
  {#if comment.address || comment.datetime}
    <footer class="mb-5 text-sm text-neutral-500 dark:text-neutral-400">
  ...
  <aside>
    ...
    {#if helpfullink || abuselink}
      <div class="flex items-center mt-3 space-x-3 divide-x divide-neutral-200 dark:divide-neutral-600">
        {#if helpfullink}
