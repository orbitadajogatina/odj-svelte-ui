<script lang="ts">
  ...
  $: wrapperClass = twMerge('w-full rounded-lg', background ? 'bg-white dark:bg-neutral-800' : 'bg-neutral-50 dark:bg-neutral-700', 'text-neutral-900 dark:placeholder-neutral-400 dark:text-white ', 'border border-neutral-200 dark:border-neutral-600', $$props.class);

  let textareaClass: string;
  $: textareaClass = wrapped ? wrappedClass : twMerge(wrapperClass, unWrappedClass);

  const headerClass = (header: boolean) => twMerge(header ? 'border-b' : 'border-t', 'py-2 px-3 border-neutral-200 dark:border-neutral-600');
