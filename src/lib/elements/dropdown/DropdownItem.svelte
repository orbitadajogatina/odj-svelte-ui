<script lang="ts">
  import Wrapper from '$lib/elements/utils/Wrapper.svelte';
  ...
  export let stay;
  onDestroy(() => {
    if (stay) document.querySelector(triggeredBy)?.focus();
    stay = false;
  })

  export let defaultClass: string = 'font-medium py-2 px-3 text-sm rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-600' + ` popper_${triggeredBy}`;
...
## Props
@prop export let defaultClass: string = 'font-medium py-2 px-4 text-sm hover:bg-neutral-100 dark:hover:bg-neutral-600';
