<script lang="ts">
  ...
  import type { Dismissable, SizeType } from '$lib/elements/types';
    ...
    <div class="flex relative {sizes[size]} w-full max-h-full">
      ...
      <Frame rounded shadow {...$$restProps} class={frameClass}>
        ...
        {#if $$slots.header || title}
          <Frame color={$$restProps.color} class="flex justify-between items-center p-4 pb-0 rounded-t-lg">
            <slot name="header">
              <h3 class="text-xl font-medium {$$restProps.color ? '' : 'text-neutral-900 dark:text-white'}">
                {title}
              </h3>
            </slot>
        ...
        {/if}
        <!-- Modal body -->
        <div class={twMerge('p-4 space-y-4 flex-1 overflow-y-auto overscroll-contain', $$props.bodyClass)} on:keydown|stopPropagation={handleKeys} role="document" on:wheel|stopPropagation|passive>
          <slot />
        </div>
        <!-- Modal footer -->
        {#if $$slots.footer}
          <Frame color={$$restProps.color} class="flex items-center p-4 space-x-2 rounded-b border-t">
            <slot name="footer" />
          </Frame>
        {/if}
      </Frame>
