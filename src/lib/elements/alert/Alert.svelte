  ...
  <Frame {...$$restProps} class={divClass} role="alert">
    ...
    {#if dismissable}
      <slot name="close-button" {close}>
        <CloseButton class="ml-auto -mr-1.5 -my-1.5" color={$$restProps.color} on:click={close} on:click on:change on:keydown on:keyup on:focus on:blur on:mouseenter on:mouseleave />
      </slot>
    {/if}
  </Frame>
{/if}
